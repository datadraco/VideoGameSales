...
fig3.update_layout(font_family="Rockwell", title_text='Global Genre Popularity', title_x=0.5, showlegend=False)

# Global ESRB trends
global_ESRB_success_df = merged_df.filter(['ESRB_Rating', 'Global_Sales']).dropna()
fig5 = px.pie(global_ESRB_success_df, values='Global_Sales', names='ESRB_Rating')
fig5.update_traces(textposition='inside', textinfo='label')
fig5.update_layout(font_family="Rockwell", title_text='Global ESRB Popularity', title_x=0.5, showlegend=False)

# Analyze global publisher success
global_publisher_success_df = merged_df.filter(['Publisher', 'Genre', 'Global_Sales']).dropna()
top_pubs = global_publisher_success_df.groupby('Publisher')['Global_Sales'].sum().reset_index()
top_10_pubs = top_pubs.sort_values(['Global_Sales'], ascending=False).head(10)
fig4 = px.bar(top_10_pubs, x='Global_Sales', y='Publisher', orientation='h', title='Global Publisher Success', template='plotly_white',
...
fig6.update_layout(font_family="Rockwell", title_text='Genre Popularity in North America', title_x=0.5, showlegend=False)
...
fig7.update_layout(font_family="Rockwell", title_text='Genre Popularity in Europe', title_x=0.5, showlegend=False)
...
fig8.update_layout(font_family="Rockwell", title_text='Genre Popularity in Japan', title_x=0.5, showlegend=False)

# Analyze publisher success across the 3 markets
regional_pub_success_df = merged_df.filter(['Publisher', 'NA_Sales', 'EU_Sales', 'JP_Sales', 'Global_Sales']).dropna()
top_pubs_regional = regional_pub_success_df.groupby('Publisher').agg({'NA_Sales':'sum', 'EU_Sales':'sum', 'JP_Sales':'sum', 'Global_Sales':'sum'}).reset_index()
top_pubs_regional = top_pubs_regional.sort_values(['Global_Sales'], ascending=False).head(10)
top_pubs_regional = top_pubs_regional.filter(['Publisher', 'NA_Sales', 'EU_Sales', 'JP_Sales'])
top_pubs_regional = top_pubs_regional.rename(columns={'NA_Sales':'NA', 'EU_Sales':'EU', 'JP_Sales':'JP'})
top_pubs_regional = pd.melt(top_pubs_regional, id_vars=['Publisher'], var_name='Region', value_name='Sales')
fig9 = px.bar(top_pubs_regional, x='Publisher', y='Sales', color='Region', barmode='group', template='plotly_white')
fig9.update_layout(font_family="Rockwell", title_text='Regional Publisher Popularity', title_x=0.5)

### Analysis Question 3 ###

# First we'll look for any correlation between the critic score and the total units shipped
critic_success_df = merged_df.filter(['Critic_Score', 'Global_Sales', 'Name'])
critic_success_df = critic_success_df.dropna()
fig1 = px.scatter(critic_success_df, x='Critic_Score', y='Global_Sales', trendline='ols', trendline_color_override="red",
                  labels={'Global_Sales': 'Global Sales (Millions)', 'Critic_Score': 'Metacritic Score'},
                  title= 'Do Critic Scores Influence Success?', template='plotly_white')
fig1.add_annotation(text='Wii Sports!', x=7.7, y=82.86, arrowhead=1, showarrow=True)
fig1.update_layout(font_family="Rockwell", title_x=0.5)

# Exclude outlier
no_wii_sports = critic_success_df[critic_success_df['Name'] != 'Wii Sports (Wii)']
fig2 = px.scatter(no_wii_sports, x='Critic_Score', y='Global_Sales', trendline= 'ols', trendline_color_override="red",
                  labels={'Global_Sales': 'Global Sales (Millions)', 'Critic_Score': 'Metacritic Score'},
                  title= 'Do Critic Scores Influence Success? (No Wii Sports)', template='plotly_white')
...
def predict_country(data):
  ...
  temp = data[['Genre', 'Platform', 'Developer', 'Critic_Score', 'ESRB_Rating', 'NA_Sales', 'EU_Sales', 'JP_Sales']]
  # Drop Nan values
  temp = temp.dropna()
  ...
  # Process data for ML
  features = temp.loc[:, ['Genre', 'Platform', 'Developer', 'Critic_Score', 'ESRB_Rating']]
...
def predict_sales(data):
  ...
  temp = data[['Genre', 'Platform', 'Developer', 'Critic_Score', 'ESRB_Rating', 'Global_Sales']]
